...
const ManageJobs = ({ setHandleAdd }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!toastId.current) {
      toastId.current = toast('Submiting please wait...', {
        position: "top-center",
        autoClose: false,
    ...
    try {
      if (location.state && location.state.action === 'edit') {
        const response = await axios.put('http://localhost:5000/auth/managejob',formData);
        toast.success(response.data.message);
    } else {
        const response = await axios.post('http://localhost:5000/auth/managejob',formData);
        toast.success(response.data.message);
    }
