...
const MyAccount = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const alumnusDetailsRes = await axios.get(`http://localhost:5000/auth/alumni?id=${alumnus_id}`);
                const coursesRes = await axios.get('http://localhost:5000/auth/courses');
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            const response = await axios.put("http://localhost:5000/auth/upaccount", formData, {
                headers: {
